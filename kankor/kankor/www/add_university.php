...
if(isset($_POST['submit'])){
	...
	$crud=new crud();
	$crud->insert("universities","name,provinces_id,description","'$university','$province','$description'");
	unset($crud);
...
if(isset($_POST['update'])){
	...
	$id=$_POST['id'];
	$crud=new crud();
	$crud->update("universities","name='$university',provinces_id='$province',description='$description'","id='$id'");
	unset($crud);
	header("location:add_university.php");
	exit();
...
if(isset($_GET['edit'])){
	...
	$editid=$_GET['edit'];
    $crud=new crud();
	$rs=$crud->select("universities","*","id='$editid'");
	$name=$rs[0]['name'];
	$provinces_id=$rs[0]['provinces_id'];
			...
			<div class="page-bar ">
				<ul class="page-breadcrumb">
					...
					<li>
						اضافه نمودن پوهنتون
							...
							action="add_university.php" method="post"
							>
								<div class="form-group">
									<label for="university" class="col-md-2 control-label">نام پوهنتون</label>
					...
					<input type="text" class="form-control" id="university" name="university" placeholder="نام پوهنتون"
					value="<?php if(isset($name)){echo $name;}?>" required
							...
							<div class="form-group">
						<label for="province" class="col-md-2 control-label">ولایت</label>	
							<div class="col-md-4">
							<select name="province" class="form-control select2me" required>
				...
				name="<?php if(isset($name)){echo "update";}else{echo"submit";} ?>" class="btn blue-madison"><?php if(isset($name)){echo "ویرایش";}else{echo"ذخیره";} ?></button>
						...
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-book"></i>لیست پوهنتون های اضافه شده
							</div>
						</div>
						<div class="portlet-body">
							...
							$provences=$rs[$i]['provinces_id'];
							
							$rs_p=$crud->select("provinces","*","id='$provences'");
							echo "<tr>
							<td>".($i+1)."</td>
							<td>".$rs[$i]['name']."</td>
							<td>".$rs_p[0]['name']."</td>
							<td>".$rs[$i]['description']."</td>
							<td><a href='add_university.php?edit=".$rs[$i]['id']."'><i class='fa fa-pencil-square-o'></i></a></td>
							<td>
							<a href='add_university.php?delid=".$rs[$i]['id']."'
							class='delete-row'
							>
